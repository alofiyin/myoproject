...
#数据接口-资讯库
#
dbhostinfo = "base.mysql.biz72_news"
...
def corp_insert(*args,**kw):
	db			= kw.pop("db")
	data		= kw.pop('data')
	res,desc 	= db.query("select max(id) as mid from news_info")
	if res == -1 return err_handle.db_err
	maxid		= res[0]['mid'] + 1
	tableName 	= getEditTableName(maxid,'news_info')
	res,des 	= db.insert(tableName,data)
	return res,des
...
def corp_update(*args,**kw):
	...
	data		= kw.pop('data')
	id 			= int(kw.pop("id"))
	tableName 	= getEditTableName(id,'news_info')
	return db.update(tableName,data,"id=%s"%id)	
