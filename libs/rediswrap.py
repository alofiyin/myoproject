...
'''
def setup_redis(name, host, port, db=0, recover_func=None,decode_flag=True, **kw):
    
    startup_nodes = [{"host": host, "port": port}] 
    SYSTEMS[name] = RedisCluster(startup_nodes=startup_nodes, decode_responses=decode_flag)
    RECOVER_FUNCS[name] = recover_func

def reg_redis(name,info):
    HOST_INFO[name]=info
...
def get_redis_handl(name='default',decode_flag=True):
    startup_nodes=HOST_INFO[name]
    return RedisCluster(startup_nodes=startup_nodes, decode_responses=decode_flag)
...
def expire(key_name,expiration,system='default'):
    """设置key的生存时间"""
    get_redis(system).expire(key_name, expiration)
    
def hincrby(name,value=1,system='default',serialized_type='string'):
    """取自增序号"""
    return get_redis(system).hincrby(name,value)
    
#---serialize data type----------------------------------------
def _convert_persistent_obj(obj):
    # fix json.dumps raise TypeError
    # 是persistent 对象
    if isinstance(obj, (UserDict, dict)):
        return dict(obj)
    elif isinstance(obj, (UserList, list, set)):
        return list(obj)
...
def json_dumps(item):
    return json.dumps(item, sort_keys=True, \
                ...
                default=_convert_persistent_obj)
...
dump_method = {'json':json_dumps,
               'pickle':pickle.dumps,
               'string':str
               }
load_method = {'json':json_loads,
               'pickle':pickle.loads,
               'string':str
               }
def check_key(key):
...
class HashFu:
    ...
    @ha_redis
    def mget(self,_keys,default=[]):
        value = get_redis(self.system).hmget(self.name, _keys)
        try:
            return list(map(self.loads, value))
        except: return default          
    @ha_redis
    def items(self):
        for key in self.keys():
            value = self.get(check_key(key))
